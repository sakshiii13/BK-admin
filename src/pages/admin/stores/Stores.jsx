...
import { createStoreApi, getAllStoresApi, updateStoreApi } from "../../../api/admin.api";
...
const Stores = () => {
  ...
  const handleOpenModal = (store = null) => {
    ...
    if (store?.location) {
      try {
        const parsed = typeof store.location === "string" ? JSON.parse(store.location) : store.location;
  ...
  // Fix default marker icon issue with leaflet in many bundlers
  ...
  const validateStoreForm = () => {
    ...
    if (!latRaw || !lngRaw) return "Store location latitude and longitude are required";
    const lat = parseFloat(latRaw);
    const lng = parseFloat(lngRaw);
    if (!Number.isFinite(lat) || !Number.isFinite(lng)) return "Enter valid numeric latitude and longitude";
  ...
  const handleCreateStore = async (e) => {
    ...
    try {
      ...
      const latNum = Number(String(formData.locationLat || "").trim());
      const lngNum = Number(String(formData.locationLng || "").trim());
      formDataObj.append("lat", latNum);
      formDataObj.append("lng", lngNum);
  ...
  const handleUpdateStore = async (e) => {
    ...
    try {
      ...
      formDataObj.append("deliveryRadius", formData.deliveryRadius);
      formDataObj.append("description", formData.description);
      const latNumU = Number(String(formData.locationLat || "").trim());
      const lngNumU = Number(String(formData.locationLng || "").trim());
      formDataObj.append("lat", latNumU);
      formDataObj.append("lng", lngNumU);
      if (Array.isArray(formData.images) && formData.images.length > 0) {
        formData.images.forEach((file) => formDataObj.append("images", file));
      }
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl max-w-2xl w-full p-8 max-h-[90vh] overflow-y-auto border border-slate-200">
            <div className="flex justify-between items-center mb-8">
              <div>
                ...
                {!isCreating && (
                  <p className="text-[#64748b] font-semibold mt-1">{selectedStore?.name}</p>
                )}
              </div>
              <button
            ...
            <form onSubmit={isCreating ? handleCreateStore : handleUpdateStore} className="space-y-6">
              {/* Basic Info Section */}
              <div className="border-b border-slate-100 pb-6">
                <h3 className="text-sm font-black uppercase text-[#64748b] tracking-wider mb-4">Basic Information</h3>
                  ...
                  <div className="mb-6">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
                      <div>
                        ...
                        <p className="text-xs text-slate-500">Click the map to set latitude and longitude automatically.</p>
                      </div>
                      <button
                        type="button"
              ...
              <div className="border-b border-slate-100 pb-6">
                <h3 className="text-sm font-black uppercase text-[#64748b] tracking-wider mb-4">Contact Information</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-black uppercase text-[#64748b] tracking-wider pl-1 block mb-2">
                      Phone Number
              ...
              <div className="border-b border-slate-100 pb-6">
                <h3 className="text-sm font-black uppercase text-[#64748b] tracking-wider mb-4">Operation Hours</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-black uppercase text-[#64748b] tracking-wider pl-1 block mb-2">
                      Opening Time
              ...
              {/* Delivery Section */}
              <div className="pb-6">
                <h3 className="text-sm font-black uppercase text-[#64748b] tracking-wider mb-4">Delivery Settings</h3>
                <div>
                  <label className="text-xs font-black uppercase text-[#64748b] tracking-wider pl-1 block mb-2">
                    Delivery Radius (KM)
                  </label>
              ...
              <div className="flex gap-4 pt-6 border-t border-slate-100">
                ...
                >
                  {updating ? (isCreating ? "Creating..." : "Updating...") : isCreating ? "Create Store" : "Update Store"}
                </button>
              </div>
            </form>
          </div>
